...
import { $ as partialMatchKey, G as hydrate, S as QueryClientProvider, V as notifyManager, W as dehydrate, Z as matchQuery, q as hashKey, y as IsRestoringProvider } from "./modern-ARvfX_Gv.js";
...
//#endregion
//#region node_modules/@tanstack/query-persist-client-core/build/modern/retryStrategies.js
...
function experimental_createQueryPersister({ storage, buster = "", maxAge = 1e3 * 60 * 60 * 24, serialize = JSON.stringify, deserialize = JSON.parse, prefix = PERSISTER_KEY_PREFIX, refetchOnRestore = true, filters }) {
	...
	async function persistQueryByKey(queryKey, queryClient) {
		if (storage != null) {
			...
			else console.warn("Could not find query to be persisted. QueryKey:", JSON.stringify(queryKey));
		}
	}
	...
	async function persisterGc() {
		...
		} else throw new Error("Provided storage does not implement `entries` method. Garbage collection is not possible without ability to iterate over storage items.");
	}
	async function restoreQueries(queryClient, filters2 = {}) {
		...
		} else throw new Error("Provided storage does not implement `entries` method. Restoration of all stored entries is not possible without ability to iterate over storage items.");
	}
	async function removeQueries(filters2 = {}) {
		...
		} else throw new Error("Provided storage does not implement `entries` method. Removal of stored entries is not possible without ability to iterate over storage items.");
	}
	return {
		persisterFn,
		persistQuery,
...
//#endregion
export { PERSISTER_KEY_PREFIX, PersistQueryClientProvider, experimental_createQueryPersister, persistQueryClient, persistQueryClientRestore, persistQueryClientSave, persistQueryClientSubscribe, removeOldestQuery };
