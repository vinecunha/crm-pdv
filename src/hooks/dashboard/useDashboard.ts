...
import { fetchDashboardDataByRole } from '@services/dashboard/dashboardService'
...
// Baseado em: public.products (top products)
...
interface UseDashboardReturn extends DashboardStats {
  rawData: RawDashboardData | undefined
  isLoading: boolean
  error: Error | null
...
export const useDashboardData = () => {
  ...
  return useQuery<RawDashboardData>({
    queryKey: ['dashboard', profile?.id, profile?.role],
    queryFn: () => fetchDashboardDataByRole(profile?.id as string, profile?.role as string),
    enabled: !!profile,
    staleTime: 5 * 60 * 1000,
...
export const useDashboardStats = (rawData: RawDashboardData | undefined): DashboardStats => {
  return useMemo(() => {
    if (!rawData) {
      return {
        primaryStats: [],
    ...
    const salesToday = sales.filter(sale => new Date(sale.created_at as string) >= today)
    const salesYesterday = sales.filter(sale => {
      const saleDate = new Date(sale.created_at as string)
      return saleDate >= yesterday && saleDate < today
    })
    const salesThisMonth = sales.filter(sale => new Date(sale.created_at as string) >= startOfMonth)
    ...
    salesThisMonth.forEach(sale => {
      const dateKey = new Date(sale.created_at as string).toISOString().split('T')[0]
...
export const useDashboard = (): UseDashboardReturn => {
  ...
  const stats = useDashboardStats(rawData)
