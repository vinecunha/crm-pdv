...
interface UseCouponsHandlersProps {
  ...
  selectedCouponForCampaign: Customer | null
  setSelectedCouponForCampaign: (customer: Customer | null) => void
  couponToDelete: Coupon | null
  setCouponToDelete: (coupon: Coupon | null) => void
  allowedCustomers: AllowedCustomer[]
  setShowModal: (show: boolean) => void
  setShowCustomersModal: (show: boolean) => void
  setShowCampaignModal: (show: boolean) => void
  setShowDeleteConfirmModal: (show: boolean) => void
  createMutation: MutationResult<{ couponData: CouponData; allowedCustomers: number[] }>
  updateMutation: MutationResult<{ id: number; couponData: CouponData; allowedCustomers: number[] }>
  deleteMutation: MutationResult<number>
  toggleStatusMutation: MutationResult<{ id: number; currentStatus: boolean }>
  addCustomerMutation: MutationResult<{ couponId: number; customer: Customer }>
  removeCustomerMutation: MutationResult<{ couponId: number; customerId: number }>
  showFeedback: (type: 'success' | 'error' | 'info', message: string) => void
}

interface UseCouponsHandlersReturn {
  handleOpenModal: (coupon?: Coupon | null) => void
  handleOpenCustomersModal: (coupon: Coupon) => void
  handleOpenCampaignModal: (coupon: Coupon) => void
  ...
  handleSendCampaign: (message: string, couponCode: string, channel: string) => Promise<void>
  closeModal: () => void
  closeCustomersModal: () => void
...
export const useCouponsHandlers = ({
  ...
  showFeedback
}: UseCouponsHandlersProps): UseCouponsHandlersReturn => {
  
  const queryClient = useQueryClient()

  const handleOpenModal = useCallback((coupon: Coupon | null = null) => {
    ...
    if (coupon) {
      ...
      if (!coupon.is_global && allowedCustomers.length > 0) {
        setSelectedCustomers(allowedCustomers.map(ac => ac.customer_id as number))
      }
    } else {
  ...
  const handleSendCampaign = useCallback(async (message: string, couponCode: string, channel: string) => {
    ...
    try {
      ...
      logger.error('❌ [Coupons] Erro ao enviar campanha', { error: (error as Error).message })
      showFeedback('error', `Erro ao enviar: ${(error as Error).message}`)
    }
