    ...
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-8">
          <div className="flex items-center justify-between">
            ...
            <div className="flex gap-3">
              <Button variant="outline" onClick={fetchUsers} loading={loading} icon={RefreshCw}>Atualizar</Button>
              {canCreateUser && (
                <Button onClick={() => { resetForm(); setShowModal(true) }} icon={Plus}>Novo Usuário</Button>
              )}
            </div>
          </div>
        </div>
        ...
        {!loading && filteredUsers.length === 0 && (
          <DataEmptyState title="Nenhum usuário encontrado" description={searchTerm || filters.role ? "Tente ajustar os filtros" : "Clique em 'Novo Usuário' para adicionar"} icon="users" />
        )}
        ...
        {!loading && filteredUsers.length > 0 && (
          <>
            ...
            <div className="hidden lg:block">
              <UserTable users={filteredUsers} currentUserId={profile?.id} onEdit={handleEdit} onDelete={(u) => { setUserToDelete(u); setShowDeleteModal(true) }} canEdit={canEditUser} canDelete={canDeleteUser} isAdmin={isAdmin} />
            </div>
          </>
        )}

        <Modal isOpen={showModal} onClose={() => !isSubmitting && setShowModal(false)} title={editingUser ? 'Editar Usuário' : 'Novo Usuário'}>
          <UserForm editingUser={editingUser} formData={formData} setFormData={setFormData} onSubmit={handleSubmit} onCancel={() => setShowModal(false)} isSubmitting={isSubmitting} canChangeRole={canChangeRole} />
        </Modal>

        <UserDeleteModal isOpen={showDeleteModal} onClose={() => setShowDeleteModal(false)} user={userToDelete} onConfirm={handleDelete} isSubmitting={isSubmitting} />
      </div>
    </div>
  )
}
