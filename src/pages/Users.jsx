...
const unlockAllUsers = async (userIds) => {
...
const Users = () => {
  ...
  const updateStatusMutation = useMutation({
    ...
    onSuccess: async (data, variables) => {
      await logAction({
        ...
        entityType: 'profile',
        entityId: data.id,
        details: { old_status: variables.oldStatus, new_status: data.status, updated_by: profile?.email }
      })
      queryClient.invalidateQueries({ queryKey: ['users'] })
  ...
  const unlockMutation = useMutation({
    ...
    onSuccess: async (data) => {
      await logAction({
        ...
        entityType: 'profile',
        entityId: data.id,
        details: { unlocked_by: profile?.email }
      })
      queryClient.invalidateQueries({ queryKey: ['blocked-users'] })
  ...
  const unlockAllMutation = useMutation({
    mutationFn: unlockAllUsers,
    onSuccess: async (count) => {
      await logAction({
        action: 'UNLOCK_ALL_USERS',
        entityType: 'profile',
        details: { unlocked_by: profile?.email, count }
      })
      queryClient.invalidateQueries({ queryKey: ['blocked-users'] })
  ...
  const handleUnlockUser = (user) => {
    unlockMutation.mutate({ id: user.id, status: 'active' })
  }
  ...
  const isMutating = createMutation.isPending || updateMutation.isPending || 
                     ...
                     unlockMutation.isPending || unlockAllMutation.isPending
          ...
          <div className="flex items-center justify-between">
            ...
            {activeTab === 'users' && (
              <div className="flex gap-3">
                <Button variant="outline" onClick={() => refetchUsers()} loading={loadingUsers} icon={RefreshCw}>
                  Atualizar
                </Button>
                {canCreateUser && (
                  <Button onClick={() => { resetForm(); setShowModal(true) }} icon={Plus} disabled={isMutating}>
                    Novo Usuário
                  </Button>
                )}
              </div>
            ...
            {activeTab === 'unlock' && (
              <div className="flex gap-3">
                <Button variant="outline" onClick={() => refetchBlocked()} loading={loadingBlocked} icon={RefreshCw}>
                  Atualizar
                </Button>
                {unlockStats.totalBlocked > 0 && (
                  <Button variant="success" onClick={() => setShowUnlockAllModal(true)} icon={Unlock} disabled={isMutating}>
              ...
              <div className="flex items-center gap-3">
                <div className="flex-1 relative">
                  ...
                  <input
                    ...
                    onChange={(e) => setUnlockSearchTerm(e.target.value)}
            ...
            >
              <div className="space-y-4">
                ...
                <div className="flex gap-3 pt-4">
                  <Button variant="outline" onClick={() => setShowUnlockAllModal(false)} disabled={unlockAllMutation.isPending} className="flex-1">
                    Cancelar
                  </Button>
                  <Button variant="success" onClick={handleUnlockAll} loading={unlockAllMutation.isPending} className="flex-1">
                    Confirmar
                  </Button>
                </div>
              </div>
