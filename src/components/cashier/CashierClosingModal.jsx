...
import { Banknote, CreditCard, QrCode, ChevronRight, AlertCircle } from 'lucide-react'
...
const CashierClosingModal = ({ 
  ...
  const paymentInputs = [
    { key: 'cash', label: 'Dinheiro', icon: Banknote, color: 'text-green-600' },
    { key: 'credit_card', label: 'Cartão Crédito', icon: CreditCard, color: 'text-blue-600' },
    { key: 'debit_card', label: 'Cartão Débito', icon: CreditCard, color: 'text-purple-600' },
  ...
  return (
    <Modal isOpen={isOpen} onClose={() => !loading && onClose()} title="Fechamento de Caixa" size="lg" isLoading={loading}>
      <div className="space-y-6">
        <div className="bg-gray-50 rounded-lg p-4">
          ...
          <div className="space-y-2">
            <div className="flex justify-between">
              ...
              <span className="font-medium">{formatDate(dateRange.start)} a {formatDate(dateRange.end)}</span>
            </div>
            <div className="flex justify-between">
            ...
            <div className="flex justify-between pt-2 border-t">
              ...
              <span className="font-bold text-green-600">{formatCurrency(expectedTotal)}</span>
            </div>
          </div>
        </div>
        ...
        <div className={`p-4 rounded-lg ${difference === 0 ? 'bg-green-50' : Math.abs(difference) < 10 ? 'bg-yellow-50' : 'bg-red-50'}`}>
          <div className="flex justify-between items-center">
            <div><p className="text-sm font-medium">Total Declarado</p><p className="text-xl font-bold">{formatCurrency(totalDeclared)}</p></div>
            <ChevronRight size={24} className="text-gray-400" />
            <div><p className="text-sm font-medium">Valor Esperado</p><p className="text-xl font-bold">{formatCurrency(expectedTotal)}</p></div>
            <ChevronRight size={24} className="text-gray-400" />
            <div><p className="text-sm font-medium">Diferença</p><p className={`text-xl font-bold ${getDifferenceColor(difference)}`}>{formatCurrency(difference)}</p></div>
          </div>
