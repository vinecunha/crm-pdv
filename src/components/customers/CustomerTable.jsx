...
import { User, Phone, Filter, Gift, Star, TrendingDown, Calendar, MessageSquare, Edit, Trash2 } from '../../lib/icons'
...
const CustomerTable = ({ customers, onEdit, onDelete, onCommunicate, onSendCampaign }) => {
  ...
  // ✅ Labels para o tooltip RFV (definido UMA vez aqui)
  const rfvLabels = {
    'A1': 'VIP / Campeão',
    'A2': 'Leal',
    'B1': 'Potencial',
  ...
  const getFilteredCustomers = useMemo(() => {
    ...
    switch(filterType) {
      ...
      case 'vip':
        return customers.filter(c => 
          c.rfv_score === 'A1' || c.rfv_score === 'A2' || c.rfv_score === 'B1'
        )
  ...
  const getRfvBadge = (score) => {
    ...
    const colors = {
      ...
      'A3': 'bg-emerald-50 text-emerald-700 border-emerald-200', 

      'B1': 'bg-blue-100 text-blue-800 border-blue-300',
      'B2': 'bg-blue-50 text-blue-700 border-blue-200',
      'B3': 'bg-sky-50 text-sky-700 border-sky-200', 
      
      'C1': 'bg-yellow-100 text-yellow-800 border-yellow-300',
      'C2': 'bg-orange-100 text-orange-800 border-orange-300',
  ...
  const getFilterCount = (type) => {
    ...
    switch(type) {
      ...
      case 'vip':
        return customers.filter(c => c.rfv_score === 'A1' || c.rfv_score === 'A2' || c.rfv_score === 'B1').length
      case 'inactive':
        return customers.filter(c => c.rfv_recency > 30 || c.rfv_score?.startsWith('C')).length
      case 'no_purchase':
        return customers.filter(c => !c.last_purchase || c.total_purchases === 0).length
      default:
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (row) => {
        ...
        return (
          <div>
            <div>{formatDate(row.last_purchase)}</div>
            {days > 30 && (
    ...
    },
    {
      ...
      render: (row) => (
        <div className="flex items-center gap-2">
          <button
            onClick={(e) => { e.stopPropagation(); onCommunicate(row) }}
            className="p-2 hover:bg-blue-50 rounded-lg transition-colors group"
            title="Comunicar"
          >
            <MessageSquare size={16} className="text-blue-500 group-hover:text-blue-600" />
          </button>
          
          {(isBirthday(row) || row.rfv_score?.startsWith('C')) && (
            <button
              onClick={(e) => { e.stopPropagation(); onSendCampaign(row) }}
              className="p-2 hover:bg-pink-50 rounded-lg transition-colors group"
              title={isBirthday(row) ? "Campanha de Aniversário" : "Campanha de Reconquista"}
            >
              <Gift size={16} className="text-pink-500 group-hover:text-pink-600" />
            </button>
          )}
          ...
          <button
            ...
            title="Editar"
          >
            <Edit size={16} className="text-gray-500 group-hover:text-gray-700" />
          </button>
          ...
          return (
            <button
              ...
              className={`
                ...
                ${isActive 
                  ? (colorClasses[option.color] || 'bg-blue-100 text-blue-700 border-blue-300')
